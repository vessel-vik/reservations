...
export default function POSInterface({ initialProducts, initialCategories }: POSInterfaceProps) {
    ...
    const handleSaveOrderChanges = async () => {
        ...
        try {
            ...
            try {
                await printOrderDocket(editingOrder.$id);
            } catch (printError) {
                console.error("Failed to print docket after order update:", printError);
            }
            ...
            <ClosedOrdersModal
                ...
                onDeleteOrder={async (order) => {
                    try {
                        ...
                        if (response.ok) {
                            toast.success('Closed order deleted');
                        } else {
                            throw new Error(payload?.error || 'Delete failed');
                        }
                    } catch (error) {
                        console.error(error);
                        toast.error('Failed to delete closed order');
                    }
                }}
            />
                ...
                onDelete={() => {
                    ...
                    void fetch(`/api/pos/orders?orderId=${recentOrder.$id}`, {
                        ...
                        if (response.ok) {
                            ...
                            toast.error(payload?.error || 'Failed to delete order');
                        }
                        setIsRecentOrderModalOpen(false);
                        setRecentOrder(null);
                    }).catch((err) => {
