...
export function SettleTableTabModal({
  ...
  const handleUpdateOrderItems = async (orderId: string, itemId: string) => {
    try {
      setError(null);
      setSuccessMessage(null);
      const order = summary.orders.find((order: any) => order.$id === orderId);
      if (!order) return;
      ...
      if (updatedItems.length === 0) {
        const response = await fetch(`/api/pos/orders?orderId=${orderId}`, { method: "DELETE" });
        if (!response.ok) {
          const payload = await response.json().catch(() => ({}));
          throw new Error(payload.error || "Failed to delete order after item removal.");
        }
        setSuccessMessage("Order removed after last item was deleted.");
  ...
  const handlePaystackFlow = async (orderIds: string[], amount: number) => {
    ...
    return new Promise<string>((resolve, reject) => {
      popup.resumeTransaction(initResult.access_code, {
        ...
        onError: (err: any) => reject(err?.message ? new Error(err.message) : new Error("Paystack failed.")),
      });
    });
  };

  const handleSettleOrders = async (orderIds: string[]) => {
    try {
      setError(null);
      setSuccessMessage(null);
  ...
  const handlePayOrder = async (orderId: string) => {
    await handleSettleOrders([orderId]);
  };
                          ...
                          <div className="space-y-4 pt-4">
                            {items.length === 0 ? (
                              ...
                              <div className="space-y-3">
                                {items.map((item: any) => (
                                  <div key={item.$id} className="flex flex-wrap items-start justify-between gap-3 rounded-3xl bg-slate-900/80 p-4">
                                    <div className="min-w-0">
                                    ...
                                    <div className="flex flex-col items-end gap-2">
                                      ...
                                      <button
                                        type="button"
                                        onClick={() => handleUpdateOrderItems(order.$id, item.$id)}
          ...
          <div className="space-y-4">
            <div className="rounded-[2rem] border border-white/10 bg-slate-950/70 p-5">
              ...
              <ul className="mt-4 space-y-3 text-sm text-neutral-300">
                ...
                <li>• Remove items from pending receipts when needed.</li>
              </ul>
            </div>
